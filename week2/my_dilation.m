function y=my_dilation(image, SE)
...
%     ii=floor(Lx/2)+1:i_x+Lx-2-floor(Lx/2);
%     jj=floor(Ly/2)+1:i_y+Ly-2-floor(Ly/2);
%     
%     y(ii,jj)=max(max(SE.*ext_image(ii-floor(Lx/2):ii+floor(Lx/2),jj-floor(Ly/2):jj+floor(Ly/2))));
